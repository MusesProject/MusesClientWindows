 ...
 */
public class MockUpPackageSensor implements ISensor {
    private static final String TAG = MockUpPackageSensor.class.getSimpleName();

    // sensor identifier
    public static final String TYPE = "CONTEXT_SENSOR_PACKAGE";

    // time in milliseconds when the sensor polls information
    private static int OBSERVATION_INTERVALL = 22000;
    ...
    private class CreateContextEventAsync {
        ...
        private Void doInBackground() throws IOException {
            while (sensorEnabled) {
                createContextEvent("torrent", "1.3", PackageStatus.INSTALLED);
                try {
