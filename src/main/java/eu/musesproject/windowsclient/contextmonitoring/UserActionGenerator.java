...
import eu.musesproject.windowsclient.contextmonitoring.sensors.AppSensor;
import eu.musesproject.windowsclient.contextmonitoring.sensors.DeviceProtectionSensor;
import eu.musesproject.windowsclient.contextmonitoring.sensors.PackageSensor;
import eu.musesproject.windowsclient.contextmonitoring.sensors.RecursiveFileSensor;
 ...
 *
 * Class to create a User action based on context events and to transform a {@link eu.musesproject.windowsclient.model.Action}
 * from a MUSES aware app to an {@link Action} that is readable for
 * the decision maker
 */
public class UserActionGenerator {
    /**
     * Method to create a user action based on the fired context events from the Sensors
     * ({@link eu.musesproject.windowsclient.contextmonitoring.sensors.ISensor}) by aggregating the context events to higher
    ...
    public static Action createUserAction(ContextEvent contextEventTrigger) {
        ...
        if(type.equals(AppSensor.TYPE)) {
        	...
        	if(prop.get(RecursiveFileSensor.PROPERTY_KEY_FILE_EVENT).equals(RecursiveFileSensor.DELETE)) {
            	action.setTimestamp(System.currentTimeMillis());
    ...
    /**
     * Method to transform a {@link Action} from a MUSES
     * aware application to an {@link Action}
     * @return {@link Action}
     */
    public static Action transformUserAction(Action musesAwareAction) {
        Action transformedAction = new Action();
        transformedAction.setActionType(transformToActionType(musesAwareAction.getActionType()));
